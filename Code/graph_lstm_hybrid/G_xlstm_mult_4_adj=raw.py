...
def plot_adjacency_heatmap(adj: np.ndarray, output_path: str, title: str, vmax_abs: float = 1.0):
    """Plot a CxC adjacency matrix heatmap (paper-friendly).

    This expects the *correlation* adjacency returned by compute_raw_adjacency_for_sample,
    which is symmetric and typically in [-1, 1].
    """
...
def apply_graph_hparams_to_branch_configs(branch_configs, graph_hidden: int | None, graph_layers: int | None):
    updated = []
    for cfg in branch_configs:
        cfg = dict(cfg)
...
def main():
    ...
    # Optional: skip feature-importance during CV (saves time)
    ...
    # Ensure graph hyperparameters actually take effect (branch_configs overrides cfg.get defaults).
    ...
    try:
        ...
        for scale_idx, cfg in enumerate(branch_configs):
            ...
            adj_mat = adjs[scale_idx][example_idx, example_window]
            out_path = os.path.join(run_dir, f'adj_example_{label}_sample{example_idx}_win{example_window}.png')
            plot_adjacency_heatmap(
                adj_mat,
                out_path,
                title=f'Adjacency (corr) - {label} - sample {example_idx}, window {example_window}',
                vmax_abs=1.0,
            )
    ...
    if compute_importance:
        print('\nComputing feature importance on full dataset (not part of CV)...')
        print('Note: enable this only if you have a chosen final model checkpoint.')
        # Intentionally left as an opt-in block to avoid mixing CV evaluation with post-hoc interpretation.
